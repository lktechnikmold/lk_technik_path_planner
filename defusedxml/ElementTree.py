...
import sys
import warnings
# Dieses Modul IST die sichere Kapselung von xml.etree.ElementTree (DOCTYPE/
# externe Entities werden unten in DefusedXMLParser abgelehnt) - der Zugriff
# auf die stdlib ist hier also die Absicherung selbst, kein ungesichertes
# Parsen fremder Daten.
...
def parse(source, parser=None, forbid_dtd=False, forbid_entities=True, forbid_external=True):
    ...
    # parser defaults to DefusedXMLParser above, which rejects DOCTYPE/entities
    return _parse(source, parser)  # nosec B314
